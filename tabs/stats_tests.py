...
def render(df_filtered):
    ...
    if len(south) > 1 and len(west) > 1:
        ...
        st.warning("⚠️ Not enough data for South and West.")
    ...
    if len(groups) > 1 and all(len(g) > 1 for g in groups):
        ...
        st.success("✅ Regions differ significantly" if p_val < 0.05 else "ℹ️ No significant difference")
    else:
        st.warning("⚠️ Need at least two regions with data for ANOVA.")
